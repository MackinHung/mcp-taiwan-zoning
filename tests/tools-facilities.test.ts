...
describe('queryPublicFacilities', () => {
  ...
  it('returns error for invalid facility_type (SQL injection protection)', async () => {
    const result = await queryPublicFacilities(env, {
      latitude: 25.033,
      longitude: 121.535,
      facility_type: "'; DROP TABLE users; --",
    });
    expect(result.isError).toBe(true);
    expect(result.content[0].text).toContain('不支援的設施類型');
    expect(result.content[0].text).toContain('park/school/road/market/parking/hospital');
  });
});
