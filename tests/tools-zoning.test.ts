...
describe('listUrbanZones', () => {
  ...
  it('returns error for invalid zone_type (SQL injection protection)', async () => {
    const result = await listUrbanZones(env, { city: '臺北市', zone_type: "' OR '1'='1" });
    expect(result.isError).toBe(true);
    expect(result.content[0].text).toContain('不支援的分區類型');
    expect(result.content[0].text).toContain('residential/commercial/industrial');
  });
