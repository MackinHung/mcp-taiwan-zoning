...
export async function listUrbanZones(
  ...
  try {
    ...
    const validationError = validateZoneInput(cityArg);
    ...
    const result = await fetchZoneStats(zoneType);
    if ('content' in result) return result; // Error case
    const data = result;

    if (!data.features || data.features.length === 0) {
      return { content: [{ type: 'text', text: `${city} 查無分區統計資料${zoneType ? `（篩選：${zoneType}）` : ''}` }] };
    }

    return formatZoneStatistics(data.features, city, zoneType);
  } catch (err) {
    return {
...
function validateZoneInput(cityArg: string): ToolResult | null {
  if (!cityArg) {
    return { content: [{ type: 'text', text: '錯誤：請提供城市名稱（city 參數）' }], isError: true };
  }
  return null;
}

function fetchZoneStats(zoneType?: string): Promise<any> | ToolResult {
  ...
  if (zoneType) {
    ...
    const code = typeMap[zoneType.toLowerCase()];
    if (!code) {
      return {
        content: [{ type: 'text', text: '錯誤：不支援的分區類型，請使用 residential/commercial/industrial' }],
        isError: true,
      };
    }
  ...
  return fetchArcGis(ARCGIS_LAYERS.ZONING, {
    where,
    outFields: '*',
...
function formatZoneStatistics(features: any[], city: string, zoneType?: string): ToolResult {
