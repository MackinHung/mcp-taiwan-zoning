...
export async function queryPublicFacilities(
  ...
  try {
    ...
    const validationError = validateFacilityInput(latitude, longitude, radiusMeters);
    if (validationError) return validationError;

    const where = buildFacilityTypeFilter(facilityType);
    if (typeof where !== 'string') return where; // Error case
    ...
    return formatFacilityResults(data.features, latitude, longitude, city, radiusMeters, facilityType);
  } catch (err) {
    return {
...
function validateFacilityInput(latitude: number, longitude: number, radiusMeters: number): ToolResult | null {
  ...
  if (radiusMeters <= 0 || radiusMeters > 5000) {
    return { content: [{ type: 'text', text: '錯誤：搜尋半徑必須在 1 至 5000 公尺之間' }], isError: true };
  }
  return null;
}

function buildFacilityTypeFilter(facilityType?: string): string | ToolResult {
  if (!facilityType) return '1=1';
  ...
  const mapped = typeMap[facilityType.toLowerCase()];
  if (!mapped) {
    return {
      content: [{ type: 'text', text: '錯誤：不支援的設施類型，請使用 park/school/road/market/parking/hospital' }],
      isError: true,
    };
  }
  return `NAME LIKE '%${mapped}%' OR ZONE_NAME LIKE '%${mapped}%'`;
}

function formatFacilityResults(
  features: any[],
  latitude: number,
  longitude: number,
  city: string,
