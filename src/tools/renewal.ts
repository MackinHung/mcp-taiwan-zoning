...
export async function queryUrbanRenewalAreas(
  ...
  try {
    ...
    const validationError = validateRenewalInput(cityArg);
    ...
    const data = await fetchRenewalData(status);

    if (!data.features || data.features.length === 0) {
      return { content: [{ type: 'text', text: `${city} 查無都市更新/重劃區資料${status ? `（狀態篩選：${status}）` : ''}` }] };
    }

    return formatRenewalResults(data.features, city, status);
  } catch (err) {
    return {
...
function validateRenewalInput(cityArg: string): ToolResult | null {
  if (!cityArg) {
...
function fetchRenewalData(status?: string): Promise<any> {
  let where = '1=1';
  if (status) {
  ...
  return fetchArcGis(ARCGIS_LAYERS.URBAN_RENEWAL, {
    where,
    outFields: '*',
...
function formatRenewalResults(features: any[], city: string, status?: string): ToolResult {
